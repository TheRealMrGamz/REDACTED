...
public class EnhancedLightSwitch : InteractableBase
{
    ...
    [SerializeField] private float flickerOnDuration = 3f;   // How long the flicker-on effect lasts
    [SerializeField] private float flickerOffDuration = 3f;  // How long the flicker-off effect lasts
    ...
    public void StartFlickerOn(float duration = -1f)
    {
        if (!isFlickering)
        {
            // Use the parameter if provided, otherwise use the serialized value
            float actualDuration = duration > 0 ? duration : flickerOnDuration;
            flickerCoroutine = StartCoroutine(FlickerRoutine(actualDuration, true));
    ...
    /// <summary>
    /// Causes the light to flicker for a specified duration and then turns it off
    /// </summary>
    /// <param name="duration">Optional: Override the default flicker duration</param>
    public void StartFlickerOff(float duration = -1f)
    {
        if (!isFlickering)
        {
            // Use the parameter if provided, otherwise use the serialized value
            float actualDuration = duration > 0 ? duration : flickerOffDuration;
            flickerCoroutine = StartCoroutine(FlickerRoutine(actualDuration, false));
    ...
    /// <summary>
    /// Immediately stops the flickering effect and sets the lights to the specified state
    /// </summary>
    /// <param name="turnOn">Whether to turn the light on (true) or off (false)</param>
    public void StopFlicker(bool turnOn = true)
    {
    ...
    public void TurnOff()
    {
        isOn = false;
        SetObjectStates();
        interactionPrompt = offPrompt;
    }

    private IEnumerator FlickerRoutine(float duration, bool endStateOn)
    {
        isFlickering = true;
